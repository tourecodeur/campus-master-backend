...
import com.campusmaster.web.dto.crud.MessagePriveRequest;
...
public class MessagePriveCrudController {
    ...
    public MessagePriveCrudController(MessagePriveRepository messageRepository, UtilisateurRepository utilisateurRepository) {
    ...
    public ResponseEntity<MessagePrive> modifier(@PathVariable Long id, @RequestBody MessagePriveRequest req) {
        return messageRepository.findById(id).map(existing -> {
            if (req.getContenu() != null) existing.setContenu(req.getContenu());
