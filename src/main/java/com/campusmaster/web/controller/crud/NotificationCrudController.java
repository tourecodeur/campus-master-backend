...
import com.campusmaster.web.dto.crud.NotificationRequest;
...
public class NotificationCrudController {
    ...
    public NotificationCrudController(NotificationRepository notificationRepository, UtilisateurRepository utilisateurRepository) {
    ...
    public ResponseEntity<Notification> creer(@RequestBody NotificationRequest req) {
        ...
        if (req.getLu() != null) n.setLu(req.getLu());
    ...
    public ResponseEntity<Notification> modifier(@PathVariable Long id, @RequestBody NotificationRequest req) {
        return notificationRepository.findById(id).map(existing -> {
            if (req.getTypeEvenement() != null) existing.setTypeEvenement(req.getTypeEvenement());
            if (req.getMessage() != null) existing.setMessage(req.getMessage());
            if (req.getLu() != null) existing.setLu(req.getLu());
            if (req.getUtilisateurId() != null) {
                Utilisateur u = utilisateurRepository.findById(req.getUtilisateurId())
                        .orElseThrow(() -> new IllegalArgumentException("Utilisateur introuvable"));
