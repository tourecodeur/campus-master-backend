...
import com.campusmaster.web.dto.crud.DevoirRequest;
...
public class DevoirCrudController {
    ...
    public ResponseEntity<Devoir> modifier(@PathVariable Long id, @RequestBody DevoirRequest req) {
        return devoirRepository.findById(id).map(existing -> {
            if (req.getTitre() != null) existing.setTitre(req.getTitre());
            if (req.getConsigne() != null) existing.setConsigne(req.getConsigne());
            if (req.getDateLimite() != null) existing.setDateLimite(req.getDateLimite());
            if (req.getCoursId() != null) {
                Cours c = coursRepository.findById(req.getCoursId())
                        .orElseThrow(() -> new IllegalArgumentException("Cours introuvable"));
                existing.setCours(c);
    ...
    public ResponseEntity<Void> supprimer(@PathVariable Long id) {
        if (!devoirRepository.existsById(id)) return ResponseEntity.notFound().build();
