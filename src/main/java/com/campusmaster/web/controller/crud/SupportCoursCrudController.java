...
import com.campusmaster.web.dto.crud.SupportCoursRequest;
...
public class SupportCoursCrudController {
    ...
    public ResponseEntity<SupportCours> creer(@RequestBody SupportCoursRequest req) {
        ...
        if (req.getVersion() != null) s.setVersion(req.getVersion());
    ...
    public ResponseEntity<SupportCours> modifier(@PathVariable Long id, @RequestBody SupportCoursRequest req) {
        return supportRepository.findById(id).map(existing -> {
            // Le nom du fichier est porté par nomFichier (titre est conservé pour compat éventuelle)
            ...
            if (req.getUrlFichier() != null) existing.setUrlFichier(req.getUrlFichier());
            if (req.getVersion() != null) existing.setVersion(req.getVersion());
            if (req.getCoursId() != null) {
                Cours c = coursRepository.findById(req.getCoursId())
                        .orElseThrow(() -> new IllegalArgumentException("Cours introuvable"));
                existing.setCours(c);
    ...
    public ResponseEntity<Void> supprimer(@PathVariable Long id) {
        if (!supportRepository.existsById(id)) return ResponseEntity.notFound().build();
